// cofre-voice-ledger.js — Cofre's Telegram entry-point workflow.
//
// Imports via: mcp__n8n-mcp__create_workflow_from_code({ code: <this file> })
//
// Why no LangChain Agent node:
//   The OpenRouter account credential already exists in n8n. OpenRouter's API
//   is OpenAI-compatible, so we hit it directly through the httpRequest
//   node with a Bearer Authorization header. The LangChain Agent route
//   would require wiring memory + an LLM subnode AND a separate outputParser
//   subnode — three subnodes for what is one HTTP POST. We avoid that.
...
// ----- HMAC verify (no network in Code node) -----------------------
const verifyHmac = node({
  ...
  output: [{ message: { chat: { id: 0 }, from: { id: 0 } } }]
});
  ...
  config: {
    ...
    parameters: {
      ...
      jsCode:
        ...
        "const chatId = nodeJson(tgWebhook, 'message.chat.id');\n" +
...
// Voice entry subtree intentionally omitted from this deploy — see header.
// For this round, every incoming message (text or voice) flows through the
// text-only entry chain: combineTranscript reads message.text and ignores
// .voice. Re-add later by restoring the inner branch on a new ifHasVoice.
...
const writeEntries = node({
  ...
  config: {
    ...
    parameters: {
      ...
      contentType: 'json',
      specifyBody: 'json',
      jsonBody: '={{ JSON.stringify([$("Build Row").item.json]) }}',
...
const synthesizeTts = node({
  type: 'n8n-nodes-base.httpRequest',
  version: 4.4,
  config: {
    name: 'Synthesize Voice (Google TTS)',
    parameters: {
      method: 'POST',
      ...
      specifyBody: 'json',
      jsonBody: expr(
        "={{ JSON.stringify({ input: { ssml: '<speak>' + $('Compose Confirmation').item.json.text.replace(/[<>]/g, c => ({'<':'&lt;','>':'&gt;'}[c])) + '</speak>' }, voice: { languageCode: $('Compose Confirmation').item.json.lang === 'bn' ? 'bn-IN' : 'en-IN', name: $('Compose Confirmation').item.json.lang === 'bn' ? 'bn-IN-Standard-A' : 'en-IN-Neural2-A' }, audioConfig: { audioEncoding: 'MP3' } }) }}"
      ),
      options: {
        response: {
          response: {
...
const sendReplyAudio = node({
  type: 'n8n-nodes-base.telegram',
  version: 1.2,
  config: {
    name: 'Send Reply Audio',
    parameters: {
      resource: 'message',
      operation: 'sendAudio',
      ...
      additionalFields: {
        caption: expr("{{ $('Compose Confirmation').item.json.text }}"),
...
export default workflow('cofre-voice-ledger', 'Cofre Voice Ledger')
  .add(tgWebhook)
  .to(verifyHmac)
  .to(ifStart)
  .onTrue(
...
// Single error fan-in to Respond 500.
...
synthesizeTts.onError(respondError);
writeTtsBinary.onError(respondError);
sendReplyAudio.onError(respondError);
